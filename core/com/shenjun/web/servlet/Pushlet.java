...
import com.shenjun.plugins.pushlet.PushletNotifyFactory;
...
public class Pushlet extends HttpServlet implements Protocol {
	...
	public  Log log = LogFactory.getLog(Pushlet.class);
	...
	protected void doRequest(Event anEvent, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if("00000".equals(Lc.getUser().getUserNo())){
				throw new Throwable("USER_INFO not find.");
			}
			...
			Lc.getUser().setPushletSession(session);
			
//			if(Lc.getUser().getPushletSession()==null||Lc.getUser().getPushletSession().getId() != session.getId() ){
//				if(Lc.getUser().getPushletSession()!=null){
//					log.warn(Lc.getUser().getPushletSession().getId()+"<->"+session.getId());
//				}else{
//					log.warn(session.getId()+"---> user getPushletSession is null");
//				}
//				Lc.getUser().setPushletSession(session);
//				
//				//request.getSession().setAttribute(User.USER_INFO, Lc.getUser());
//			}
			
			
			PushletNotifyFactory factory=SpringUtil.getBean("pushletNotifyFactory", PushletNotifyFactory.class);
			if(factory!=null){
				factory.execute(session,anEvent);
			}else{
				log.warn("pushletNotifyFactory not find.");
			}
