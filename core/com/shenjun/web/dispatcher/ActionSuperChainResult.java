...
*/
public class ActionSuperChainResult implements Result {

	public  Log log = LogFactory.getLog(ActionSuperChainResult.class);
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	private static final Logger LOG = LoggerFactory.getLogger(ActionSuperChainResult.class);
    ...
    public ActionSuperChainResult() {
        super();
    }

    public ActionSuperChainResult(String namespace, String actionName, String methodName) {
        this.namespace = namespace;
        this.actionName = actionName;
        this.methodName = methodName;
    }

    public ActionSuperChainResult(String namespace, String actionName, String methodName, String skipActions) {
        this.namespace = namespace;
        this.actionName = actionName;
        this.methodName = methodName;
    ...
    public void setData(String url) {
		...
		String[] ps=url.split("\\?");
			if(ps.length==2){
				this.actionName = ps[0].substring(ps[0].lastIndexOf("/")+1,ps[0].length()-7);
				this.namespace = ps[0].substring(0,ps[0].lastIndexOf("/"));
				log.info("namespace["+this.namespace+"]actionName["+this.actionName+"]");
				String[] parameters=ps[1].split("&");
				for(String parameter : parameters){
					String[] para=parameter.split("=");
					Struts2Util.set(para[0], para[1]);
					//log.info(para[0]+"->"+para[1]);
				}
			}
    	}catch (Exception e) {
    ...
    public void execute(ActionInvocation invocation) throws Exception {
        ...
        if (isInChainHistory(finalNamespace, finalActionName, finalMethodName)) {
            ...
            throw new XWorkException("Infinite recursion detected: "
                    + ActionSuperChainResult.getChainHistory().toString());
        }

        if (ActionSuperChainResult.getChainHistory().isEmpty() && invocation != null && invocation.getProxy() != null) {
        ...
        extraContext.put(CHAIN_HISTORY, ActionSuperChainResult.getChainHistory());
    ...
    @Override public boolean equals(Object o) {
        ...
        final ActionSuperChainResult that = (ActionSuperChainResult) o;
    ...
    private boolean isInChainHistory(String namespace, String actionName, String methodName) {
        LinkedList<? extends String> chainHistory = ActionSuperChainResult.getChainHistory();
    ...
    private void addToHistory(String namespace, String actionName, String methodName) {
        List<String> chainHistory = ActionSuperChainResult.getChainHistory();
